...
def d(matching, preflist):
    # calculate summation of ranks
    d_m, d_w, n = 0, 0, len(preflist[0])
    for m, w in enumerate(matching):
        rank_m = preflist[0][m].index(w)
        rank_w = preflist[1][w].index(m)
        d_m += rank_m
        d_w += rank_w
    return (d_m + d_w)/(2*n)

def e(matching, preflist):
    # calculate disparity
    sum_of_ranks_men, sum_of_ranks_women = 0, 0
    for i, match in enumerate(matching):
        rank_m = preflist[0][i].index(match)
        rank_w = preflist[1][match].index(i)
        sum_of_ranks_men += rank_m
        sum_of_ranks_women += rank_w
    return np.abs(sum_of_ranks_men - sum_of_ranks_women)
